...
# script = "hybrid_6_asylum_support_base"
script = "hybrid_6_admin_review_last_far_outcome"
...
with open("C:\\Users\\samukhia\\OneDrive - Capgemini\\Desktop\\Work\\Home "
          "Office\\Hybrid Guides\\admin_review_sql_scripts\\" + script + ".sql") as f:
    data = f.read()
...
class BusinessRules:
    ...
    def remove_end(self):
        return re.sub(r"END|END\s+\w+", "", self.business_rule)
    ...
    def replace_dateadd(self):
        ...
        while keep_search == 1:
            try:
                ...
                days_to_add = re.search(r",\s\d+,|,\d+,", original_dateadd).group(0)
    ...
    def replace_else(self):
        return re.sub(r"(\s)?(\n)?ELSE\s", ", otherwise ", self.business_rule)
    ...
    def replace_in(self):
        ...
        while keep_search == 1:
            try:
                ...
                self.business_rule = self.business_rule.replace(original_in, transformed_in)
            except AttributeError:
                keep_search = 0
                pass
    ...
    def replace_count(self):
        ...
        while keep_search == 1:
            try:
                ...
                transformed_count = re.sub(r"COUNT\(", "count of ", original_count)
    ...
    def replace_row_number(self):
        return re.sub(r"ROW_NUMBER\(\)", r"row number of the field", self.business_rule)

    def replace_order(self):
        self.business_rule = re.sub(r"DESC", "in descending order", self.business_rule)
        self.business_rule = re.sub(r"ASC", "in ascending order", self.business_rule)
        return re.sub(r"ORDER BY", "and order by", self.business_rule)
...
def apply_business_rule_transformer(business_rule):
    ...
    tr18 = BusinessRules(tr17).replace_row_number()
    tr19 = BusinessRules(tr18).replace_order()
    output = re.sub(r"^\s+", "", tr19)
    output = re.sub(r"\n\s+", "\n", output)
...
df.to_csv("C:\\Users\\samukhia\\OneDrive - Capgemini\\Desktop\\Work\\Home "
          "Office\\Hybrid Guides\\admin_review_sql_scripts\\" + table + ".csv")
attr.to_csv("C:\\Users\\samukhia\\OneDrive - Capgemini\\Desktop\\Work\\Home "
          "Office\\Hybrid Guides\\admin_review_sql_scripts\\Attributes_" + table + ".csv" )
